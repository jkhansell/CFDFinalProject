...
def experimental_data_validation(coefs, liftdata, dragdata, angles, Re):
    ...
    with open("metrics"+Re+"reduced.json", "w") as outfile:
...
if __name__ == "__main__":
    ...
    print(coefs.shape)    
    experimental_data_validation(coefs[1:-2], cl_data, cd_data, angles, Re)
    ...
    ax.plot(angles, coefs[1:-2,2], marker=".", label=r"$\gamma-Re_\theta$  SST simulation")
    ...
    ax.plot(angles, coefs[1:-2,1], marker=".", label=r"$\gamma-Re_\theta$ SST simulation")
    ...
    ax.plot(angles, coefs[1:-2,2]/coefs[1:-2,1], marker=".", label=r"$\gamma-Re_\theta$ SST simulation")
    ax.set_xlabel("Angle of attack $[^\circ]$", fontsize=16)
    ax.set_ylabel(r"$C_d\quad[-]$", fontsize=16)
    ax.legend()
